...
func NewTrimCommand() Command {
	args, alias, desc := MakeArgs(
		[]string{"input", "k", "end_threshold", "middle_threshold", "absolute_threshold", "discard_middle", "check_reads", "adapter_threshold", "extra_end_trim", "extra_middle_trim", "tag_adapters", "verbosity", "front_adapters", "back_adapters", "num_workers"},
		[]string{"", "6", "75", "85", "25", "false", "10000", "90", "5", "100", "true", "1", "", "", "4"},
		[]string{"Fasta/fastq/gzip input file", "k-mer size to use when matching adapters", "% identity for matching adapters at read edges", "% identity for matching adapters that split reads", "Identity (in bases) above which all adapters match", "Whether to keep halves of split reads", "Number of reads to use to determine which adapters are present", "% identity required at check_adapters stage", "Number of bases to remove around adapters at read edges", "Number of bases to remove around read-splitting adapters", "Whether to add adapter names to output sequence names", "Level (0-2) of output to stderr", "Fasta/fastq file containing front adapters", "Fasta/fastq file containing back adapters", "Number of threads to use"})
